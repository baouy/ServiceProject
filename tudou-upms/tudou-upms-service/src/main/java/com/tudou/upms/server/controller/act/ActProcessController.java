...
import com.tudou.upms.server.controller.act.service.ActProcessService;
...
public class ActProcessController extends BaseController {
	...
	public Object runningList(@ModelAttribute ProcessRunValid processRunValid) {
		...
		return new UpmsResult(UpmsResultConstant.SUCCESS,list,pc,ps,processRunValid.getMaxnum());
	}
	...
	public Object convertToModel(String procDefId) throws UnsupportedEncodingException, XMLStreamException {
		...
		return new UpmsResult(UpmsResultConstant.SUCCESS,"转换模型成功，模型ID="+modelData.getId());
	}
	...
	public Object deploy(String category, MultipartFile file) {
		...
		if (num == 0){
			return new UpmsResult(UpmsResultConstant.FAILED,"部署失败");
		}else if(num == 2){
			return new UpmsResult(UpmsResultConstant.FAILED,"不支持的文件类型");
		}else{
			return new UpmsResult(UpmsResultConstant.SUCCESS,"操作成功");
		}
	}
