 ...
 */
package com.tudou.upms.server.controller.act.utils;
...
public class ProcessDefCache {
	...
	public static ProcessDefinition get(String procDefId) {
		String name = ACT_CACHE + ACT_CACHE_PD_ID_ + procDefId;
		ProcessDefinition pd = (ProcessDefinition) SerializeUtil.deserialize(RedisUtil.get(name.getBytes()));
		if (pd == null) {
			RepositoryService repositoryService = SpringContextUtil.getNBean(RepositoryService.class);
			pd = repositoryService.createProcessDefinitionQuery().processDefinitionId(procDefId).singleResult();
			if (pd != null) {
				RedisUtil.set(name.getBytes(), SerializeUtil.serialize(pd));
			}
		}
		return pd;
	}
