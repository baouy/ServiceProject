...
import com.tudou.upms.server.controller.act.utils.ProcessDefCache;
...
public class ActTaskService{
	...
	public List<ActTaskValid> todoList(ActTaskValid act){
		...
		// =============== 已经签收的任务  ===============
		TaskQuery todoTaskQuery = taskService.createTaskQuery().taskAssignee(upmsUser.getUserId().toString()).active()
		...
		// =============== 等待签收的任务  ===============
		TaskQuery toClaimQuery = taskService.createTaskQuery().taskCandidateUser(upmsUser.getUserId().toString())
