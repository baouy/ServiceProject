package com.tudou.upms.client.shiro.filter;
...
import com.tudou.upms.client.util.RequestParameterUtil;
import com.tudou.upms.common.constant.UpmsConstant;
...
public class UpmsAuthenticationFilter extends AuthenticationFilter {
    ...
    // 局部会话key
    private final static String TUDOU_UPMS_CLIENT_SESSION_ID = "tudou-upms-client-session-id";
    // 单点同一个code所有局部会话key
    private final static String TUDOU_UPMS_CLIENT_SESSION_IDS = "tudou-upms-client-session-ids";
    ...
    private boolean validateClient(ServletRequest request, ServletResponse response) {
        ...
        // 判断局部会话是否登录
        String cacheClientSession = RedisUtil.get(TUDOU_UPMS_CLIENT_SESSION_ID + "_" + session.getId());
        if (StringUtils.isNotBlank(cacheClientSession)) {
            // 更新code有效期
            RedisUtil.set(TUDOU_UPMS_CLIENT_SESSION_ID + "_" + sessionId, cacheClientSession, timeOut);
            Jedis jedis = RedisUtil.getJedis();
            jedis.expire(TUDOU_UPMS_CLIENT_SESSION_IDS + "_" + cacheClientSession, timeOut);
            jedis.close();
            // 移除url中的code参数
            ...
            try {
                ...
                if (httpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                    ...
                    if (1 == result.getIntValue("code") && result.getString("data").equals(code)) {
                        // code校验正确，创建局部会话
                        RedisUtil.set(TUDOU_UPMS_CLIENT_SESSION_ID + "_" + sessionId, code, timeOut);
                        // 保存code对应的局部会话sessionId，方便退出操作
                        RedisUtil.sadd(TUDOU_UPMS_CLIENT_SESSION_IDS + "_" + code, sessionId, timeOut);
                        _log.debug("当前code={}，对应的注册系统个数：{}个", code, RedisUtil.getJedis().scard(TUDOU_UPMS_CLIENT_SESSION_IDS + "_" + code));
                        // 移除url中的token参数
                        String backUrl = RequestParameterUtil.getParameterWithOutCode(WebUtils.toHttp(request));
                        // 返回请求资源
                        try {
