	...
	Plugin.prototype = {
		...
		renderIconContainer: function () {
			...
			for (var i = 0, item; item = iconsPaged[i++];) {
				...
				// Set the icon box
				$('<span/>', {
					html:      '<i class="layui-icon" data-fip-value="' + item + '" ' + (this.settings.useAttribute ? (this.settings.attributeName + '="' + ( this.settings.convertToHex ? '&#x' + parseInt(item, 10).toString(16) + ';' : item ) + '"') : 'class="' + item + '"') + '>'+item+'</i>',
					'class':   'fip-box',
					title: flipBoxTitle
